...
def _sync_ep_model_param(model: nn.Module):
    ...
    if parallel_context.is_initialized(ParallelMode.DATA) and parallel_context.get_world_size(ParallelMode.DATA) > 1:
        ...
        if 1 in param_dict:
            ...
            for param in param_dict[1]:
                dist.broadcast(param, src=src_rank, group=parallel_context.get_group(ParallelMode.DATA))
...
class UniformNoiseSampler:
    ...
    def __init__(self, eps: float = 1e-2):
        ...
        self.popl = torch.distributions.uniform.Uniform(low=lower_bound, high=upper_bound).rsample
