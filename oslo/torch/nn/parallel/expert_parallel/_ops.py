...
try:
    ...
    print("If you want to activate cuda kernel for Expert Parallel, Please install with cuda_extension.")


class AllToAll(torch.autograd.Function):

    @staticmethod
    def forward(context: Any, inputs: Tensor, group: Optional[ProcessGroup] = None) -> Tensor:
...
class EPDispatch(torch.autograd.Function):
    ...
    def forward(context, tokens, mask, dest_idx, ec):
        ...
        expert_input = oslo_expert_parallel_cuda.dispatch_forward(s, ec, h, tokens, mask, dest_idx)
    ...
    def backward(context, output_grad):
        ...
        d_tokens = oslo_expert_parallel_cuda.dispatch_forward(context.s, context.ec, context.h,
                                                              output_grad, mask, dest_idx)
...
class EPCombine(torch.autograd.Function):
    ...
    def forward(context, expert_tokens, logits, mask, dest_idx, ec):
        ...
        ctokens = oslo_expert_parallel_cuda.combine_forward(s, e, c, h, combine_inp, logits, mask, dest_idx)
    ...
    def backward(context, tokens_grad):
        ...
        combine_grad = tokens_grad.to(torch.float32) if tokens_grad.type is torch.float16 else tokens_grad

        combine_inp = expert_tokens.to(torch.float32) if context.fp16_flag else expert_tokens
        d_expert, d_logits = oslo_expert_parallel_cuda.combine_backward(context.s, context.e, context.s, context.h,
                                                                        combine_grad, combine_inp,
